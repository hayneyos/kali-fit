...
from backend.app_recipe.utils.base.mongo_client import ingredient_names_collection, ingredients_nutrition_collection, \
    ingredient_categories_collection, normalized_ingredients_collection
...
def clean_text(text: str, remove_common_words: bool = True) -> str:
    ...
    text = re.sub(
        r'\d+(?:\s*\/\s*\d+)?\s*(?:ounce|oz|g|kg|ml|l|sliced|slice|tablespoons|tablespoon|cups|cup|tbsp|tsp|pieces|piece|cans|can|packs|pack|chunks|chunk|)?',
        '', text)
...
def find_similar_names(names_data: List[Dict], similarity_threshold: float = 0.8, batch_size: int = 1000) -> Dict[
    str, List[Dict]]:
    """
...
def create_normalized_collection():
    ...
    try:
        ...
        categories_dict = {str(doc['_id']): doc for doc in categories_data}
        ...
        for group_key, group_items in tqdm(grouped_names.items(), desc="Creating documents"):
            ...
            # Get category info from the first item in the group
            first_item = group_items[0]
            category_id = first_item.get('category_id')
            category_info = categories_dict.get(str(category_id), {})
            ...
            for item in group_items:
                ...
                if 'english' in cleaned_names:
                    cleaned_names['english']['name'] = clean_text(cleaned_names['english']['name'],
